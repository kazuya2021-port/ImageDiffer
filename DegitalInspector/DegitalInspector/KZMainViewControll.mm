...
@interface KZMainViewControll()<NSTabViewDelegate>
{
    NSString *tabLabelBase;
    NSMutableDictionary *tblDiffViewController;
}
@property (nonatomic, weak) IBOutlet NSTabView *mainTab;
@property (nonatomic, weak) IBOutlet PSMTabBarControl *tabBarView;
- (void)onChangeHotFolderState:(id)obj keyPath:(NSString*)keyPath change:(NSDictionary*)change;
@end

@implementation KZMainViewControll
...
- (id)init
{
    self = [super init];
    if(self){
        tblDiffViewController = [NSMutableDictionary dictionary];
        tabLabelBase = NSLocalizedStringFromTable(@"TabLabelBase", @"MainUI", nil);
    }
    return self;
}
...
- (void)loadView
{
    ...
    [self addNewDiff];
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(onChangeHotFolderState:) name:@"NotifyChangeHotFolderState" object:nil];
}
...
{
    NSString *countStr = tabViewItem.identifier;
    [tblDiffViewController removeObjectForKey:countStr];
    /*NSString *countStr = tabViewItem.identifier;
    
    [_mainTab removeTabViewItem:tabViewItem];
    */
}
...
- (void)addNewDiff
{
    NSUInteger tabCount = [tblDiffViewController count] + 1;
    NSString *countStr = [NSString stringWithFormat:@"%lu",tabCount];
    NSString *label = [tabLabelBase stringByAppendingString:countStr];

    NSTabViewItem *mainTabItem = [[NSTabViewItem alloc] initWithIdentifier:countStr];
    mainTabItem.label = label;
    KZDiffView *dv = [[KZDiffView alloc] initWithNibName:@"KZDiffView" bundle:nil];
    dv.logContentView = _logTextWindow.contentView;
    [dv loadView];
    
    NSView *theView = [dv view];
...
{
    KZDiffView *dv = nil;
    for (NSTabViewItem *item in _mainTab.tabViewItems) {
        for (NSView *v in item.view.subviews) {
            if ([v isKindOfClass:[KZDiffView class]]) {
                dv = (KZDiffView *)v;
                break;
            }
        }
    ...
    [dv.diffImg stopEngine];
}

#pragma mark -
#pragma mark Notify
- (void)onChangeHotFolderState:(NSNotification*)notify
{
    BOOL isRunning = [[notify object] boolValue];
    NSDictionary *infoDic = [NSBundle.mainBundle infoDictionary];
    NSString *appName = infoDic[@"CFBundleName"];
    if (isRunning) {
        self.window.title = [NSString stringWithFormat:@"%@ - %@", appName, NSLocalizedStringFromTable(@"HotFolderRuning", @"MainUI", nil)];
    }
    else {
        self.window.title = [NSString stringWithFormat:@"%@", appName];
    }
